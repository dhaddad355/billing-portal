...
export default function ReferralSettingsPage() {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      const [templatesRes, settingsRes] = await Promise.all([
        ...
        fetch("/api/app/settings/letter-settings"),
      ]);
  ...
  const saveTemplate = async () => {
    ...
    try {
      const url = editingTemplate
        ? `/api/app/settings/letter-templates/${editingTemplate.id}`
        : "/api/app/settings/letter-templates";
  ...
  const deleteTemplate = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/app/settings/letter-templates/${templateToDelete.id}`, {
        method: "DELETE",
      });
  ...
  const saveSettings = async () => {
    ...
    try {
      const res = await fetch("/api/app/settings/letter-settings", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
